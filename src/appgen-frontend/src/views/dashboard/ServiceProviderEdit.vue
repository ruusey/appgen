<template>
    <div>
        <v-card class="mx-auto p3"
                ...
                outlined>
            <v-toolbar dark flat>
                <v-toolbar-title v-if="s.id">Edit Serivice Provider</v-toolbar-title>
                <v-toolbar-title v-if="!client.id">New Serivice Provider</v-toolbar-title>
            </v-toolbar>
            <v-card-text>
                <v-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteItem(){
                ServiceProviderService.destroy(this.sp.id).then(res => {
                    this.$store.dispatch('ADD_NOTIFICATION', {
                        text: 'Client Deleted' ,
                        color: 'primary'
                    });
