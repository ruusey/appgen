...
class AppgenApplicationTests {
	...
	public void checkTest() throws JsonProcessingException {
		GeoLocation geoLoc1 = new GeoLocationBuilder().withDateTime(Instant.now().toString()).withLat(33.753746)
				.withLng(-84.386330).build();
		GeoLocation geoLoc2 = new GeoLocationBuilder().withDateTime(Instant.now().toString()).withLat(33.753746)
				.withLng(-84.386330).build();
		GeoLocation geoLoc3 = new GeoLocationBuilder().withDateTime(Instant.now().toString()).withLat(33.753746)
				.withLng(-84.386330).build();
		JobRequest job = new JobRequestBuilder().withComplete(false).withLoc(geoLoc3)
				.withShortDescrtion("Wanted software engineer").withLongDescrtion("long-desc").withPay(100000.0)
				.build();
		ServiceProvider sp = new ServiceProviderBuilder().withEmail("ruusey@gmail.com").withFirstName("Robert")
				.withLastName("Usey").withUserName("ruusey").withRating(100f).build();
		Client client = new ClientBuilder().withEmail("ruusey@gmail.com").withFirstName("Robert").withLastName("Usey")
				.withUserName("ruusey").withRating(100f).build();
